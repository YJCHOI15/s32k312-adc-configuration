...
1. **LED 출력 핀 (`PTC10/PTB17/PTB16`):**
    ...
    2. **SIUL2.MSCRn**
        ...
        ![image.png](attachment:aae8f330-d8e4-46b4-a0e1-e96d11bf2b95:image.png)
        
        - `SSS[2:0]` 를 ‘GPIO’ 기능으로 설정
...
**FIRC_CLK(48MHz) → 2 분주 → CORE_CLK(24MHz) → ADC0 공급**
...
1. **ADC_0.MCR**
    ...
    `NSTART` - 일반 변환을 시작시키는 트리거 역할을 한다. 단일 변환 모드일 경우 (`MODE` = 0) 변환 후 필드 값은 자동으로 0이 된다. 
    ...
    `ADCLKSEL` - 클럭 신호 주파수를 선택 (CORE_CLK → AD_CLK), 반드시 파워 다운 상태(`PWDN` = 1)일 때만 값을 쓸 수 있다. 
